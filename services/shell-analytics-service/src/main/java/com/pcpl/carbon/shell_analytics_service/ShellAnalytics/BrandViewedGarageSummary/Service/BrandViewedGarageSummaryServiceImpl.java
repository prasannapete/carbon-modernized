...
@Slf4j
@Service
public class BrandViewedGarageSummaryServiceImpl extends AbstractLazyService<BrandViewedGarageSummary, BrandViewedGarageSummaryDTO, BrandViewedGarageSummaryRepository> implements BrandViewedGarageSummaryService{

    @Autowired
    BrandViewedGarageSummaryRepository brandViewedGarageSummaryRepository;
    ...
    @Override
    public BrandViewedGarageSummary getEntityObject() {
        return new BrandViewedGarageSummary();
    }

    @Override
    public BrandViewedGarageSummaryDTO getDtoObject() {
        return new BrandViewedGarageSummaryDTO();
    }
    ...
    public ApplicationResponse getSummaryByDate(String givenDate) {
        try {
            ...
            List<Object[]> rows = brandViewedGarageSummaryRepository.getSummaryByDate(date);
            ...
            for (Object[] row : rows) {
                ...
                map.put("brandViewedCount", row[2]);
                map.put("cumulativeBrandViewedCount", row[3]);
                result.add(map);
            }
    ...
    public void generateEndOfDaySummaryForCountry(String country, Date summaryDate) {
        try {
            Long todayCount = brandViewedGarageRepository.getCountByCountryAndDate(country, summaryDate);
            if (todayCount == null) todayCount = 0L;

            if (todayCount == 0) {
                log.info("No BrandViewedGarage records for {} on {}, skipping summary", country, summaryDate);
                return;
            }

            Long cumulativeCount =
                    brandViewedGarageRepository.getCumulativeCountByCountryUpToDate(country, summaryDate);
            if (cumulativeCount == null) cumulativeCount = 0L;


            List<BrandViewedGarageSummary> existingList =
                    brandViewedGarageSummaryRepository.getSummaryByCountryAndDate(country, summaryDate);

            BrandViewedGarageSummary existing =
                    (existingList != null && !existingList.isEmpty()) ? existingList.get(0) : null;

            if (existing != null) {
                existing.setGeneratedDate(summaryDate);
                existing.setBrandViewedCount(todayCount);
                existing.setCumulativeBrandViewedCount(cumulativeCount);
                existing.setLastModifiedTime(new java.util.Date());

                brandViewedGarageSummaryRepository.save(existing);

                log.info("Updated BrandViewedGarage summary for {}: date={}, count={}, cumulative={}",
                        country, summaryDate, todayCount, cumulativeCount);

            } else {
                BrandViewedGarageSummary summary = new BrandViewedGarageSummary();
                summary.setCountry(country);
                summary.setGeneratedDate(summaryDate);
                summary.setBrandViewedCount(todayCount);
                summary.setCumulativeBrandViewedCount(cumulativeCount);
                summary.setIsDeleted(0);
                summary.setCreationTime(new java.util.Date());

                brandViewedGarageSummaryRepository.save(summary);

                log.info("Created BrandViewedGarage summary for {}: date={}, count={}, cumulative={}",
                        country, summaryDate, todayCount, cumulativeCount);
            }

        } catch (Exception e) {
            log.error("Failed to generate BrandViewedGarageSummary for {} on {}", country, summaryDate, e);
        }
    }
    ...
    public Map<String, Object> getSummariesByUserCountries(String countries, String startDateStr, String endDateStr) throws Exception {
        ...
        if (countryList.isEmpty()) {
            List<UserCountriesDTO> userCountriesDTOS =
                    userCountriesRepository.getUserCountriesByUserId(user.getId());
            countryList = userCountriesDTOS.stream()
        ...
        List<Object[]> results =
                brandViewedGarageSummaryRepository.getSummariesByDateRange(startDate, endDate, countryList);

        List<Object[]> cumulativeRows =
                brandViewedGarageSummaryRepository.getLatestCumulativeCounts(countryList);
        ...
        Map<String, Map<String, Object>> countrySummaryMap = new LinkedHashMap<>();

        for (Object[] row : results) {
            BrandViewedGarageSummary summary = (BrandViewedGarageSummary) row[0];
            ...
            long count = summary.getBrandViewedCount();

            if (count > 0) {
                countryWiseGraphData
            ...
            summaryData.put("launches", currentLaunches + count);
        ...
        for (Map<String, Object> row : summaryTable) {
            totalLaunches += (Long) row.get("launches");
            totalCumulative += (Long) row.get("cumulative");
        }
    ...
    @Override
    public void migrateBrandViewedData(String startDateStr, String endDateStr) throws Exception {

        SimpleDateFormat formatter = new SimpleDateFormat("yyyy-MM-dd");
        Date startDate = formatter.parse(startDateStr);
        Date endDate = formatter.parse(endDateStr);

        log.info("Starting Brand Viewed Garage migration from {} to {}", startDateStr, endDateStr);
        ...
        List<Object[]> rows = brandViewedGarageRepository.getDailyCounts(startDate, endDate);
        log.info("Found {} records to migrate", rows.size());

        int processedCount = 0;
        int batchSize = 50;
        List<BrandViewedGarageSummary> batch = new ArrayList<>();
        ...
        for (Object[] row : rows) {
            ...
            List<BrandViewedGarageSummary> existingList =
                    brandViewedGarageSummaryRepository.findByCountryAndDate(country, generatedDate);

            BrandViewedGarageSummary summary;

            if (!existingList.isEmpty()) {
                summary = existingList.get(0);
                log.debug("      └─ Updating existing record");
            } else {
                summary = new BrandViewedGarageSummary();
                summary.setCountry(country);
            ...
            summary.setBrandViewedCount(dailyCount);
            summary.setCumulativeBrandViewedCount(newCumulative);
            ...
            if (batch.size() >= batchSize) {
                brandViewedGarageSummaryRepository.saveAll(batch);
        ...
        log.info("Brand Viewed Garage migration complete: {} records processed from {} to {}",
    ...
    private Map<String, Long> getStartingCumulatives(Date startDate) {
        ...
        log.info("Fetching brand viewed cumulative counts before {}", startDate);

        List<Object[]> priorCounts = brandViewedGarageRepository.getCumulativeCountsBeforeDate(startDate);
        ...
        if (priorCounts.isEmpty()) {
            log.info("No prior brand viewed data found. Starting cumulative from 0 for all countries.");
        }
