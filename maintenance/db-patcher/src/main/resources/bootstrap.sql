...
--changeset sharanya:cb_roles-alter-id-identity
--preconditions onFail:MARK_RAN onError:HALT
--precondition-sql-check expectedResult:f select exists(select 1 from information_schema.columns where table_schema='${carbon.database.name}' and table_name='cb_roles' and column_name='id' and is_identity='YES')
ALTER TABLE cb_roles ALTER COLUMN id ADD GENERATED BY DEFAULT AS IDENTITY;

--changeset sharanya:cb_app_features-alter-id-identity
--preconditions onFail:MARK_RAN onError:HALT
--precondition-sql-check expectedResult:f select exists(select 1 from information_schema.columns where table_schema='${carbon.database.name}' and table_name='cb_app_features' and column_name='id' and is_identity='YES')
ALTER TABLE cb_app_features ALTER COLUMN id ADD GENERATED BY DEFAULT AS IDENTITY;

--changeset sharanya:cb_role_accounts-alter-id-identity
--preconditions onFail:MARK_RAN onError:HALT
--precondition-sql-check expectedResult:f select exists(select 1 from information_schema.columns where table_schema='${carbon.database.name}' and table_name='cb_role_accounts' and column_name='id' and is_identity='YES')
ALTER TABLE cb_role_accounts ALTER COLUMN id ADD GENERATED BY DEFAULT AS IDENTITY;
